...
from dfkit.decision_tree.fitting import AUTO_MIN_SAMPLES_FLOOR, MAX_TREE_DEPTH
...
class DecisionTreeModule:
    ...
    def system_prompt(self) -> str:
        """LLM guidance for using the analyze_with_decision_tree tool.

        Returns:
            str: System prompt text explaining when and how to use the tool.
        """
        return (
            ...
            "- `max_depth`: Controls tree complexity (1-6). Start with the default (6) "
            "and reduce if the rules are too detailed.\n"
            "- `min_samples_leaf`: Minimum samples required at each leaf. Higher values "
            "reduce noise. Default is 5.\n"
